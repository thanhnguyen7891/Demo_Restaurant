...
public class Waiter {
	static String listItems[]= {"1.Full meals --100","2.veg biriyani--150", "3.veg Rice--50",
			"4.veg Palav--100", "5.veg Manchuria--50", "6.gobi Nudles--50",
			"7.chicken Biriyani--200", "8.chicken--200", "9.chicken Curry--100", 
	...
	void takeOrder(Customer customer) {
		...
		for(int i=0;i<itemsNum;i++) {
			System.out.println("itemNumber "+(i+1)+": enter no of item from 1-10");
			order.item[i][0]=sc.nextInt();
			System.out.println("itemNumber "+(i+1)+":enter quantity ");
	...
	void placeOrderToChef(Order order) {
		...
		for(int i=0;i<order.noOfItems;i++) {
			...
			if(listItems[noOrder-1].contains("veg")) {
				...
				vegChef.prepareOrder(order, "veg");
			
			}else {
				System.out.println("order number "+noOrder+" is getting ready please wait (non veg)");
				nonVegChef.prepareOrder(order, "non veg");
			}
		}
